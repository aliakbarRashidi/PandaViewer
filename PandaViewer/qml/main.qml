...
ApplicationWindow {
    ...
    theme {
        primaryColor: Qt.rgba(0.2, 0.2, 0.2, 0.9)
    ...
    initialPage: Page {
        ...
        actionBar.customContent: Item {
            ...
            Label {
                ...
                text: page.title
                style: "title"
                color: Theme.lightDark(page.actionBar.backgroundColor,
            ...
            Rectangle {
                ...
                TextField {
                    ...
                    function findWord() {
                        ...
                        searchText.end = nextSpace == -1 ? searchText.text.length : nextSpace
                        var substr = searchText.text.substring(0,
                                                               searchText.end)
                        searchText.start = substr.lastIndexOf(" ")
                        searchText.start = searchText.start
                                == 0 ? searchText.start : searchText.start + 1

                        var word = searchText.text.substring(searchText.start,
                                                             searchText.end)
                        mainWindow.askForTags(word)
                    }
                    function proccessEnter() {
                        if (autoCompleteDropdown.visible
                                && autoCompleteDropdown.view.count > 0) {
                            addTag()
                            autoCompleteDropdown.close()
                        } else {
                            searchText.accept()
                    ...
                    function addTag() {
                        var first = searchText.text.substring(0,
                                                              searchText.start)
                        var end = searchText.text.substring(
                                    searchText.end, searchText.text.length)
                    ...
                    Keys.onEscapePressed: autoCompleteDropdown.close()
            ...
            Dropdown {
                ...
                ListView {
                    ...
                    delegate: ListItem.Standard {
                        ...
                        actionWidth: Units.dp(24) - Units.dp(16)
                        action: Item {
                        }
    ...
    Component {
        ...
        Dropdown {
            ...
            property alias view: listView
            width: Units.dp(100)
            height: listView.childrenRect.height < Units.dp(
                        mainWindow.height / 2) ? listView.childrenRect.height : Units.dp(
                                                     mainWindow.height / 2)
            data: ListView {
                id: listView
                anchors.fill: parent
