...
from . import Threads, Exceptions, UserDatabase, Metadata
...
class Program(QtWidgets.QApplication, Logger):
    ...
    class SortMethodMap(Enum):
        NameSort = attrgetter("sort_name")
        ReadCountSort = attrgetter("read_count")
        LastReadSort = attrgetter("last_read")
        RatingSort = attrgetter("sort_rating")
        DateAddedSort = attrgetter("time_added")
        FilePathSort = attrgetter("sort_path")
    ...
    def set_auto_metadata_collection(self, galleries: List[GenericGallery] = None):
        ...
        for gallery in self.filter_galleries(galleries):
            ...
            assert matching_dir
            ex_auto_collection = Config.folder_options.get(matching_dir, {}).get(Config.AUTO_METADATA_KEY, True)
            if gallery.metadata_manager.get_metadata_value(
                    Metadata.MetadataClassMap.gmetadata, "auto_collection") != ex_auto_collection:
                gallery.metadata_manager.update_metadata_value(Metadata.MetadataClassMap.gmetadata,
                                                               "auto_collection",
                                                               ex_auto_collection)
    ...
    def sort(self):
        key = [t.value for t in self.SortMethodMap][Config.sort_type]
        assert key
